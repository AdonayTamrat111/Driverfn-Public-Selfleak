...
#define code_rw CTL_CODE(FILE_DEVICE_UNKNOWN, 0x82, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define code_ba CTL_CODE(FILE_DEVICE_UNKNOWN, 0x83, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define code_get_guarded_region CTL_CODE(FILE_DEVICE_UNKNOWN, 0x84, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define code_security 0x9e5f6d4 
...
namespace ioctl {
	...
	bool find_driver() {
		driver_handle = CreateFileW((L"\\\\.\\\Iud"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
T write(uint64_t address, T buffer) {
	...
	ioctl::write_physical((PVOID)address, &buffer, sizeof(T));
	return buffer;
}
...
{
	readm(reinterpret_cast<PVOID>(address), reinterpret_cast<PVOID>(array), sizeof(T) * len);
}
