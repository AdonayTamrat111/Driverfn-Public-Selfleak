...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::other::main_purple : c::other::text_purple_no_active, g.IO.DeltaTime * 8);

    RenderFrame(bb.Min, bb.Max, GetColorU32(c::input::background_active), true, 3.f);
	window->DrawList->AddRect(ImVec2(bb.Min.x - 1, bb.Min.y - 1), ImVec2(bb.Max.x + 1, bb.Max.y + 1),GetColorU32(c::input::rect), 3.f);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
   ...
   // window->DrawList->AddRectFilled(bb_frame.Min + ImVec2(8,0), bb_frame.Max, bg_col, window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x + 8, ImLerp(bb.Min.y + 5, bb.Max.y, grab_v_norm), bb.Max.x + 3, ImLerp(bb.Min.y, bb.Max.y - 5, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(c::other::main_purple), 30.f);
...
bool ImGui::Tab(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
{
    ...
    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? c::other::main_purple : ImVec4(205.f / 255.f, 216.f / 255.f, 255.f / 255.f, 1.f), g.IO.DeltaTime * 6.f);

    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? c::child::background : ImVec4(41.f / 255.f, 43.f / 255.f, 44.f / 255.f,0.f), g.IO.DeltaTime * 6.f);

    it_anim->second.text = ImLerp(it_anim->second.text, selectable ? c::other::main_purple : ImVec4(205.f / 255.f, 216.f / 255.f, 255.f / 255.f, 1.f), g.IO.DeltaTime * 6.f);
    //it_anim->second.line = ImLerp(it_anim->second.line, selectable ? c::accent_color : c::tab::line_inactive, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.background), 4);

    ImGui::GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(15, 5), bb.Max + ImVec2(-15, -25), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
  
    PushFont(Lexend_tab);
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size_arg.y - CalcTextSize(label).y) / 2 + 15), GetColorU32(it_anim->second.text), label);
    PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.Text_color = ImLerp(it_anim->second.Text_color, *v ? c::other::text_purple : c::other::text_purple_no_active, g.IO.DeltaTime * 5);
    it_anim->second.rect = ImLerp(it_anim->second.rect, *v ? ImColor(43, 43, 43, 255) : ImColor(43, 43, 43, 255), g.IO.DeltaTime * 5);
    it_anim->second.check_color = ImLerp(it_anim->second.check_color, *v ? c::checkbox::background_active : c::checkbox::background_inactive, g.IO.DeltaTime * 5);
    window->DrawList->AddRect(total_bb.Min + ImVec2(0, 0), total_bb.Min + ImVec2(16, 16), GetColorU32(it_anim->second.rect), 4.f);
    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(0, 0), total_bb.Min + ImVec2(16, 16), GetColorU32(it_anim->second.check_color), 4.f);
    ...
    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x + 23 - square_sz, check_bb.Min.y), GetColorU32(it_anim->second.Text_color), label);
...
bool ImGui::Keybind(const char* label, int* key, bool show_label)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float width = (GetContentRegionMax().x - style.WindowPadding.x - 11);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - 11 - g.Style.WindowPadding.x) - square_sz;
    ...
    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? c::other::text_purple : c::other::text_purple_no_active, ImGui::GetIO().DeltaTime * 6.f);
    it_anim->second.slow_show = ImLerp(it_anim->second.slow_show, clickable.Min.x - rect.Min.x, g.IO.DeltaTime * 15.f);
    it_anim->second.rect = ImLerp(it_anim->second.rect, g.ActiveId == id ? ImColor(43, 43, 43, 255) : ImColor(43, 43, 43, 255), g.IO.DeltaTime * 5);
    window->DrawList->AddRect(ImVec2(it_anim->second.slow_show + rect.Min.x - 1, clickable.Min.y - 3), clickable.Max - ImVec2(-1, 4), GetColorU32(it_anim->second.rect), 3.f);
    window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y - 2), clickable.Max - ImVec2(0, 5), GetColorU32(c::input::background_active), 3.f);
...
struct knob_state {
    float plus_float;
    int plus_int;
    ImVec4 background, circle, text, text_1;
...
#include <wtypes.h>
#include <D3DX11.h>
bool ImGui::KnobScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = GetContentRegionMax().x - 13 - style.WindowPadding.x;

    const ImVec2 label_size = CalcTextSize(label, NULL, true);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w, 19));
    ...
    if ((flags & ImGuiSliderFlags_Integer) == 0) {
        it_anim->second.plus_float = ImLerp(it_anim->second.plus_float, *(float*)p_data <= *(float*)p_max && hovered && GetAsyncKeyState(VK_OEM_PLUS) & 0x01 ? *(float*)p_data += 0.01f : *(float*)p_data >= *(float*)p_min && hovered && GetAsyncKeyState(VK_OEM_MINUS) & 0x01 ? *(float*)p_data -= 0.01f : 0, g.IO.DeltaTime * 6.f);
        if (*(float*)p_data > *(float*)p_max) *(float*)p_data = *(float*)p_max;
        if (*(float*)p_data < *(float*)p_min) *(float*)p_data = *(float*)p_min;
    }
    else
    {
        it_anim->second.plus_int = ImLerp(it_anim->second.plus_int, *(int*)p_data <= *(int*)p_max && hovered && GetAsyncKeyState(VK_OEM_PLUS) & 0x01 ? *(int*)p_data += 0.5 : *(int*)p_data >= *(int*)p_min && hovered && GetAsyncKeyState(VK_OEM_MINUS) & 0x01 ? *(int*)p_data -= 0.5 : 0, g.IO.DeltaTime * 6.f);
        if (*(int*)p_data > *(int*)p_max) *(int*)p_data = *(int*)p_max;
        if (*(int*)p_data < *(int*)p_min) *(int*)p_data = *(int*)p_min;
    }

    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? c::other::text_purple : c::other::text_purple_no_active, g.IO.DeltaTime * 6.f);

    it_anim->second.text_1 = ImLerp(it_anim->second.text_1, g.ActiveId == id ? c::other::text_purple : c::text::text_1, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.position = ImLerp(it_anim->second.position, *static_cast<float*>(p_data) / *reinterpret_cast<const float*>(p_max) * 6.25f, ImGui::GetIO().DeltaTime * 5.f);

    GetWindowDrawList()->PathClear();
    GetWindowDrawList()->PathArcTo(ImVec2(frame_bb.Max.x + radius - 22.f, frame_bb.Min.y + (19 / 2)), radius, 0.f, 2.f * IM_PI, 40.f);
    GetWindowDrawList()->PathStroke(GetColorU32(c::other::backgroud), 0, thickness);

    GetWindowDrawList()->PathClear();
    GetWindowDrawList()->PathArcTo(ImVec2(frame_bb.Max.x + radius - 22.f, frame_bb.Min.y + (19 / 2)), radius, IM_PI * 1.5f, IM_PI * 1.5f + it_anim->second.position, 40.f);
    GetWindowDrawList()->PathStroke(GetColorU32(c::other::main_purple), 0, thickness);

    GetWindowDrawList()->AddCircleFilled(ImVec2(frame_bb.Max.x + radius - 22.f + ImCos(IM_PI * 1.5f + it_anim->second.position) * radius, frame_bb.Min.y + (19 / 2) + ImSin(IM_PI * 1.5f + it_anim->second.position) * radius), 2.f, GetColorU32(c::text::text_active));

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - (40 + CalcTextSize(value_buf).x), frame_bb.Min.y + (19 - CalcTextSize(value_buf).y) / 2), GetColorU32(it_anim->second.text_1), value_buf);

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w, frame_bb.Min.y + (16 - CalcTextSize(value_buf).y) / 2), GetColorU32(it_anim->second.text), label);

    return value_changed;
}

bool ImGui::KnobFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return KnobScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::KnobInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 11, 26));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? c::other::text_purple : c::other::text_purple_no_active, g.IO.DeltaTime * 6.f);
    it_anim->second.icon = ImLerp(it_anim->second.icon, it_anim->second.opened_combo ? c::other::text_purple : c::input::icon, g.IO.DeltaTime * 6.f);
    ...
    window->DrawList->AddRect(ImVec2(bb.Min.x - 1, bb.Min.y - 3), ImVec2(bb.Max.x + 1, bb.Max.y), GetColorU32(c::input::rect), 3.f);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y - 2), ImVec2(bb.Max.x, bb.Max.y - 1), GetColorU32(c::input::background_active), 3.f);
   ...
   // PushFont(sf_pro_text_1);
    ...
    GetWindowDrawList()->AddImage(image::arrow, ImVec2(value_x2, bb.Min.y) + ImVec2(1, 2), bb.Max - ImVec2(9, 5), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
	PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(7, 3), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();
    // PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(w - 50, label_size.y + 17.f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, 0.0f));
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::slider::background_active : c::slider::background_inactive, g.IO.DeltaTime * 6.f);
    it_anim->second.background_rect = ImLerp(it_anim->second.background_rect, IsItemActive() ? ImColor(43, 43, 43, 255) : ImColor(43, 43, 43, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::other::text_purple : c::other::text_purple_no_active, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Max - ImVec2(5, 5), GetColorU32(c::slider::background_inactive), 100.f);
    //GetWindowDrawList()->AddRect(ImVec2(frame_bb.Min.x + 248, frame_bb.Min.y), ImVec2(frame_bb.Max.x + 41, frame_bb.Max.y + 3), GetColorU32(it_anim->second.background_rect), 3.f);

    if (grab_bb.Max.x > grab_bb.Min.x) {

        it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 25.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 8), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y - 3), GetColorU32(c::slider::background_active), 100.f);

        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y - 5), 6.f, GetColorU32(c::slider::circle_active), 100.f);
    }

    PushFont(Inters);
    window->DrawList->AddText(ImVec2(total_bb.Min.x - CalcTextSize(value_buf).x / 2 + w - 23, total_bb.Min.y + 4), GetColorU32(it_anim->second.text), value_buf);
    PopFont();

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 50, frame_bb.Min.y - 20), GetColorU32(it_anim->second.text), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGui::SetCursorPos(GetCursorPos() + ImVec2(1, -5));
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 78, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 4.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y + 8);
  
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 6), window->DC.CursorPos + frame_size + ImVec2(0, 4));
    ...
    it_anim->second.line = ImLerp(it_anim->second.line, IsItemActive() ? c::input::line : c::input::line_in, g.IO.DeltaTime * 10.f);
    it_anim->second.line1 = ImLerp(it_anim->second.line1, IsItemActive() ? frame_size.x - 137 : 0, g.IO.DeltaTime * 10.f);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::input::text : c::input::text_inactive, g.IO.DeltaTime * 10.f);
    ...
    if (!is_multiline)
    {
        ...
        //   ImGui::GetWindowDrawList()->AddShadowRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 2), ImVec2(frame_bb.Max.x, frame_bb.Max.y + 2), ImColor(202, 222, 252, 255), 45.f, ImVec2(0, 0), 0, 6.f);

        RenderFrame(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y), ImVec2(frame_bb.Max.x , frame_bb.Max.y), GetColorU32(c::input::background_active), false, 3.f);
        window->DrawList->AddRect(ImVec2(frame_bb.Min.x - 2, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), GetColorU32(c::input::rect),3.f);
    }
    if (!is_multiline)
    {
   
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + frame_size.x / 2 - it_anim->second.line1, frame_bb1.Min.y + 31), ImVec2(frame_bb1.Min.x + frame_size.x / 2 + it_anim->second.line1, total_bb1.Min.y + 33),GetColorU32(it_anim->second.line),8.f, ImDrawFlags_RoundCornersBottom);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(4, 0), rect.Max + ImVec2(4, 3), bg_color);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            // ImGui::PushFont(ExtraLight);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(5, 1), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            //  ImGui::PopFont();
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 4, 6), frame_bb.Min + ImVec2(it_anim->second.value + 5, 20), GetColorU32(c::input::cursor));
            }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImU32 col = GetColorU32(it_anim->second.text_color);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(5, 1), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    ...
    // it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? text_active : hovered ? c::text_hov : c::text, g.IO.DeltaTime * 6.f);

    //window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcItemWidth() - 73, frame_bb.Min.y), ImColor(91, 100, 114, 255), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::picker::background_inactive));
    ...
    static std::map<ImGuiID, coilor_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, coilor_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.Text_color = ImLerp(it_anim->second.Text_color, IsItemActive() ? c::other::text_purple : c::other::text_purple_no_active, g.IO.DeltaTime * 5);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 0), GetColorU32(it_anim->second.Text_color), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const float width = GetContentRegionMax().x - 10 - GetStyle().WindowPadding.x;
    const ImRect rect(pos, pos + ImVec2(width, 18));
    const ImRect clickable(pos + ImVec2(width - 30, 1), pos + ImVec2(width, 16));
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), 10, ImVec2(0.f, 0.f),3.f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::other::main_purple : c::text::texts, g.IO.DeltaTime * 6.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(text_min + ImVec2(5,5), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
